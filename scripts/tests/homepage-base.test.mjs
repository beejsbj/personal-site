...
test('approved warm welcome is the production homepage, without comparison controls', () => {
  ...
  for (const tag of tags('a')) {
    if (tag.href) {
      assert.equal(new URL(tag.href.replaceAll('&amp;', '&'), siteUrl).searchParams.has('variant'), false,
...
test('main landmark and keyboard skip destination remain accessible', () => {
  ...
  assert.ok(tags('a').some((tag) => tag.href === '#main-content'
    && tag.class?.split(/\s+/).includes('skip-link')), 'Missing skip-to-content link');
  assert.equal([...html.matchAll(/\bid=["']main-content["']/g)].length, 1,
    'Skip target must be unique');
});

test('homepage preserves canonical and social metadata', () => {
  assert.equal(tags('link').find((tag) => tag.rel === 'canonical')?.href, siteUrl.href);
  ...
  assert.equal(meta('property', 'og:image'), 'https://burooj.dev/images/burooj4.jpg');
...
test('real featured projects, current work, and dated updates remain present', () => {
  const links = [...html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)]
    .map(([, attrs, body]) => ({ ...attributes(attrs), text: plainText(body) }));
  for (const [slug, title] of [
    ['api3-ecosystem', 'API3 Ecosystem'],
    ['conduit-market', 'Conduit Market'],
    ['qrng', 'QRNG Demo Apps'],
  ]) {
    assert.ok(links.some((link) => link.href?.replace(/\/$/, '') === `/projects/${slug}`
      && link.text.includes(title)), `Missing project link: ${title}`);
  }
  for (const text of [
    'Currently', 'bjslab', 'Recent updates',
    'A stable home for the API3 showcase',
    'Conduit Market joins the portfolio',
    'The QRNG demos are playable again',
  ]) {
    assert.ok(plainText(html).includes(text), `Missing homepage content: ${text}`);
  }
  for (const date of ['2026-09-02', '2026-07-01', '2026-06-20']) {
    assert.ok(tags('time').some((tag) => tag.datetime === date), `Missing update date: ${date}`);
  }
});
...
test('local links, stylesheets, icons, and image sources resolve in built output', () => {
  ...
  for (const reference of references) {
    ...
    const candidates = [join(outputDir, pathname), join(outputDir, pathname, 'index.html')];
    assert.ok(candidates.some((path) => {
